...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'zora', 'Zora', and 'ZORA'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.zora/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.zora/g),
   ...
   * Rename files.
   *
   * Rename files that had zora in their name, such as
   * css/zora.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*zora*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('zora', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-zora-system.mjs`);
